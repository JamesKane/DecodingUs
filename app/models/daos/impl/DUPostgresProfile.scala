...
/**
 * The DUPostgresProfile trait is a Slick profile for PostgreSQL.
 *
 * It extends the ExPostgresProfile trait and provides additional support for various PostgreSQL features
 * such as array, date/time, range, hstore, full-text search, PostGIS, and JSON.
 *
 * This trait defines the MyAPI object which contains implicit type mappers for custom data types and
 * extension methods for query transformations.
 *
 * Usage:
 * - Extend this trait in your Slick profile object to get support for PostgreSQL specific features.
 * - Use the MyAPI object to access the additional functionality provided by this trait.
 */
trait DUPostgresProfile extends ExPostgresProfile
  with PgArraySupport
  with PgDate2Support
  with PgRangeSupport
...
object DUPostgresProfile extends DUPostgresProfile